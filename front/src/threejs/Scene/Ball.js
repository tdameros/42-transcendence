...
export class Ball {
    ...
    #handlePaddleCollision(travel, paddle, timeDelta) {
        const {point: topIntersection, t: topT} = travel.intersect(paddle.topCollisionSegment);
        const {point: frontIntersection, t: frontT} = travel.intersect(paddle.frontCollisionSegment);
        const {point: bottomIntersection, t: bottomT} = travel.intersect(paddle.bottomCollisionSegment);
    ...
    #addBallToGroup() {
        let ball = new THREE.Mesh(new THREE.SphereGeometry(this.#radius, 10, 10),
    ...
    #addLightToGroup() {
        let light = new THREE.PointLight(0xFFFFFF, 75.0, 40.);
