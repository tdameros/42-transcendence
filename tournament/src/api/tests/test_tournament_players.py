...
class KickPlayerTest(TestCase):
    ...
    @patch('common.src.jwt_managers.UserAccessJWTDecoder.authenticate')
    def kick_player(self, tournament_id, user_id, admin_id, mock_authenticate_request):
        user = {'id': 1}
        mock_authenticate_request.return_value = (True, user, None)
        url = reverse('kick-player', args=(tournament_id, user_id))

        response = self.client.delete(url, content_type='application/json', headers=get_fake_headers(admin_id))
    ...
    def test_kick_player(self):
        response, body = self.kick_player(1, 1, 1)
    ...
    def test_not_owner(self):
        ...
        self.assertEqual(body['errors'], [error.NOT_OWNER])
        self.assertEqual(response.status_code, 403)
